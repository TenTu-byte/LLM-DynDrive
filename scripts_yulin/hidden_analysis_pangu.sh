...
python hidden_analysis_mixed_auto_pangu.py \
  ...
  --save_path  "$DATASET_DIR/outputs_yulin_gy/test3/openPangu-Embedded-7B-V1.1/Math_Math/steer_vector_layer${LAYER_ID}_conf_mixed.pt" \
  --threshold $THRESHOLD \
  --max_files 500 \
  --expected_offset 0 \
  --device npu \
  --report_path  "$DATASET_DIR/outputs_yulin_gy/test3/openPangu-Embedded-7B-V1.1/Math_Math/analysis_layer${LAYER_ID}.json" \
  --verbose \
