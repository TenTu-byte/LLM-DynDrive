...
# Multi-node inference with seed loop
for seed_val in {27..42}; do
    echo "=== Running with seed: ${seed_val} ==="
    for ds in "${datasets[@]}"; do
        echo "=== Evaluating dataset: ${ds} with seed ${seed_val} ==="
        torchrun \
            --nnodes=$MA_NUM_HOSTS \
            ...
            --master_port=29500 \
            baseline_pangu_dist.py \
            --model_name_or_path "$models/openPangu-Embedded-7B-V1.1" \
            --dataset_dir "./Data" \
            --dataset "$ds" \
            --output_path "$outputs/outputs_baseline" \
            --max_generated_tokens $max_tokens \
            --seed $seed_val
        echo "=== Finished ${ds} with seed ${seed_val} ==="
    done
    echo "=== Completed all datasets for seed ${seed_val} ==="
done