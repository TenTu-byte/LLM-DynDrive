...
def worker(args, rank, world_size, local_rank, device):
    ...
    use_clf = (insert_clf is not None) and (meta is not None)
    ...
    for i in my_indices:
        ...
        result = {
            ...
            "inserted": bool(inserted) if use_clf else False,
            "insert_step": insert_step if use_clf else None,
            "insert_policy": "clf_multicheck" if use_clf else "no_clf",
            "clf_prob_threshold": float(clf_thr) if use_clf else None,
            "clf_layer": int(clf_layer) if use_clf else None,
            "clf_pos_is_short_meta": bool(meta_pos_is_short) if use_clf else None,
            "clf_meta_n": int(meta_n) if use_clf else None,
            "checkpoints": checkpoints if use_clf else [],
        }
...
def main():
    ...
    parser.add_argument('--max_generated_tokens', type=int, default=32000)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument("--insert_text", type=str, default="\n[unused17]\n\n")
    parser.add_argument("--hs_device", type=str, default="auto", choices=["auto", "npu", "cuda", "cpu"])
    parser.add_argument("--clf", type=str, default="", help="Path to insert_clf.joblib")
    parser.add_argument("--meta", type=str, default="", help="Path to insert_clf_meta.json")
    parser.add_argument("--clf_layer", type=int, default=None, help="Override meta.best_layer (default: use meta)")
