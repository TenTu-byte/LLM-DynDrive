        ...
        **loss_kwargs,
        ...
        if self.steering_flag and getattr(self, "_dyn_enabled", True):
            ...
            if ready_mask.any():
                ...
                # === 动态调控：用二维 F(c, v)（带边界约束）替换一维 F(c)（只改这一小段逻辑） ===
                # 1) 当前段的均值置信 mean_max（你原有逻辑算好的）
                # 2) 根据“前两段”计算方差 var_conf = ((cur - prev)**2)/4，仅在 #ready 上写入
                bsz = input_ids.shape[0]
                ...
                # 3) 一维基线 F(c)：你的可控双曲正切拟合（手动超参，和原位置一致）
                import math as _m
                ...
                q25c = 0.67   # confidence q25,1.5b:0.67
                q75c = 0.94   # confidence q75,1.5b:0.94
                low_val_1 = -1.5  # F(q25c),1.5b:-1.5
                high_val_1 = 0.01   # F(1)

                # —— 新增：方差分位 + 两个目标 —— 
                q25v = 0.000489
                q75v = 0.011
                low_val_2  = -4.0  # f(q25c, q75v)  低c+高v更低（下界）1.5b:-4
                ...
                # tensor 版门控（用于批量）
                def g_low_c_t(c):   return _sigmoid((q25c - c) / IQRc * 12.0)   # c 越低越大
                def g_high_c_t(c):  return _sigmoid((c - q75c) / IQRc * 12.0)   # c 越高越大
                def g_high_v_t(v):  return _sigmoid((v - q75v) / IQRv * 12.0)   # v 越高越大
                def g_low_v_t(v):   return _sigmoid((q25v - v) / IQRv * 12.0)   # v 越低越大

                # 标定用标量门控（与上面同形），用于归一化权重
                ...
                # —— 归一 + 封顶 的权重（∈[0,1]），确保不越过 low/high 边界 —— 
                w_low  = (g_low_c_t(mean_max)  * g_high_v_t(var_conf)) / max(gp, eps)  # 低c × 高v
                w_high = (g_high_c_t(mean_max) * g_low_v_t(var_conf))  / max(gm, eps)  # 高c × 低v
                w_low  = torch.clamp(w_low,  max=1.0)
                w_high = torch.clamp(w_high, max=1.0)

                # 5) 计算 updated 系数：基线 + 低侧权重×(目标差) + 高侧权重×(目标差)
                base = F1(mean_max)
                ...
                # reset accumulators for those samples（保持你原逻辑不变）
