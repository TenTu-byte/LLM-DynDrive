    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if self.steering_flag and getattr(self, "_dyn_enabled", True):
            ...
            if self._temp is None or self._temp.shape[0] != bsz:
                self._temp = torch.ones(bsz, dtype=torch.float32, device=device)
            ...
            if ready_mask.any():
                ...
                # —— 置信分位 ——
                q25c = 0.67   # confidence q25,1.5b:0.67,7b:0.71,32b:0.70
                q75c = 0.94   # confidence q75,1.5b:0.94,7b:0.95,32b:0.92

                # —— 方差分位 —— 
                q25v = 0.000489## 1.5b:0.000489,7b:0.000382，32b:0.0004	
                q75v = 0.011##1.5b:0.011,7b:0.009080，32b:0.0073
                ...
                cold_temp = 0.7   # 低置信 + 高方差 -> 降温（overthinking防止乱说）
                warm_temp = 1.2  # 高置信 + 低方差 -> 升温（underthinking提高解码温度）

                # 条件 1：confidence < q25c 且 v_confidence > q75v -> 降温
                cond_lowc_highv = (mean_max < q25c) & (var_conf > q75v)

                # 条件 2：confidence > q75c 且 v_confidence < q25v -> 升温
                cond_highc_lowv = (mean_max > q75c) & (var_conf < q25v)

                # 先默认 temp = 1.0
                temp_here = torch.ones_like(mean_max)

                # 第一步：对低置信 + 高方差的样本设成 cold_temp
                temp_here = torch.where(
                    cond_lowc_highv,
                    torch.full_like(mean_max, float(cold_temp)),
                    temp_here,
                )

                # 第二步：对高置信 + 低方差的样本设成 warm_temp
                temp_here = torch.where(
                     cond_highc_lowv,
                     torch.full_like(mean_max, float(warm_temp)),
                     temp_here,
                )

                
                self._temp[ready_mask] = temp_here

 
                self._coefs[ready_mask] = 0

                
                self._step_prob_sum[ready_mask] = 0.0
                self._step_tok_count[ready_mask] = 0

                # 更新“上一段均值置信”，供下一段计算 var_conf
        ...
        # === 动态温度 ===
        if self.steering_flag and getattr(self, "_dyn_enabled", True) and self._temp is not None:
            logits = logits / self._temp.view(-1, 1, 1).to(logits.dtype)
