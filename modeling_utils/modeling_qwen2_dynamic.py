        ...
        **loss_kwargs,
        ...
        if self.steering_flag and getattr(self, "_dyn_enabled", True):
            ...
            if ready_mask.any():
                ...
                # === 动态调控：用可控的 F(c) 替换原先的阈值分段逻辑 ===
                # 需求：给定 (q25, q75)、低端目标 F(q25)=low_val、顶端目标 F(1)=tau，
                #       同时固定 F(q75)=0，构造平滑的双曲正切型 F(c)，然后 updated = F(mean_max)
                def _solve_k_for_tau(q25, q75, low_val, high_val, tau, iters=80):
                    # 计算形状参数 k，使得在给定低/高端约束下 F(1)=tau
                    m = 0.5 * (q25 + q75)
                ...
                def build_F(q25, q75, low_val=-3.0, high_val=0.0, tau=0.037):
                    # 构造满足 F(q25)=low_val, F(q75)=high_val(默认0), F(1)=tau 的平滑函数
                    m = 0.5 * (q25 + q75)
                    s = max(1e-9, 0.5 * (q75 - q25))
                    ...
                    def F(c):  # 支持 float / list / numpy / torch.Tensor
                        if not isinstance(c, torch.Tensor):
                            c = torch.as_tensor(c)  # -> CPU tensor
