        ...
        for l, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if steering_flag is not None and steering_layer == l:
                ...
                if isinstance(steering_coef, torch.Tensor):
                    ...
                    coef = torch.as_tensor(steering_coef, dtype=hidden_states.dtype, device=hidden_states.device)  # TODO: no need to index steering_flag? not for batch?
...
class PanguEmbeddedForCausalLM(PanguEmbeddedPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if self.steering_flag:  # TODO: support batch or not
            if self.new_round:
                self.new_round=False
        ...
        self.cur_steps += 1  # TODO: why step?
        ...
        if self.steering_flag and getattr(self, "_dyn_enabled", True):
            ...
            if ready_mask.any():
                mean_max = self._step_prob_sum[ready_mask] / self._step_tok_count[ready_mask].float()  # TODO: not geometric average?
                ...
                var_conf[has_prev] = ((mean_max[has_prev] - prev_vals[has_prev]) ** 2) / 4.0  # TODO: unbiased estimation?
                ...
                q25c = self.q25c if self.q25c is not None else 0.74   # v0:0.75; v1:0.74; v2:0.74
                q75c = self.q75c if self.q75c is not None else 0.92   # v0:0.93; v1:0.92; v2:0.92
                low_val_1 = self.low_val_1 if self.low_val_1 is not None else -1.17  # F(q25c), v0:-1.14; v1:-0.99; v2:-1.17
                high_val_1 = self.high_val_1 if self.high_val_1 is not None else 0.01   # F(1), 0.01

                # —— 新增：方差分位 + 两个目标 —— 
                q25v = self.q25v if self.q25v is not None else 0.000270  # v0:0.000289; v1:0.000263; v2:0.000270
                q75v = self.q75v if self.q75v is not None else 0.006074  # v0:0.005617, v1:0.006008; v2:0.006074
                low_val_2 = self.low_val_2 if self.low_val_2 is not None else -4.0  # f(q25c, q75v)  v0:-5.54; v1:-3.97; v2:-4.0
            ...
            current_coef = torch.as_tensor(float(self.steering_coef), dtype=torch.float32, device=input_ids.device)  # TODO: parameter exists even w/o steering?
        # ! ------ steering ------
        ...
        if self.steering_flag and getattr(self, "_dyn_enabled", True):
            with torch.no_grad():
                ...
                self._last_maxprob = probs.max(dim=-1).values  # TODO: take token-independent max probability; mismatch?
        # ! ------ steering ------
