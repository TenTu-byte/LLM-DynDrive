...
def sample_with_tracking(
    ...
    with torch.inference_mode(), torch.npu.amp.autocast(enabled=(dtype in (torch.float16, torch.bfloat16)), dtype=dtype):
    ...
    for _ in range(max_new_tokens):
        with torch.inference_mode(), torch.npu.amp.autocast(enabled=(dtype in (torch.float16, torch.bfloat16)), dtype=dtype):
            if last_token is None:
                ...
                out = model(  # TODO: next_logprob not used?
...
def worker(rank, world_size, args):
    ...
    for i in my_indices:
        ...
        # text_before_think = response_text.split('[unused17]')[0]  # ! adapt to pangu
