...
def _token_pattern(tok: str) -> str:
    ...
    def word2regex(w: str) -> str:
        if w == "verify":
            return r"verif(?:y|ies|ied|ying|ication(?:s)?)"  # create nested non-capturing groups
        if w == "alternative":
    ...
    return rf"\b{body}\b"  # add word boundary
...
def batch_build_all_mixed(
    ...
    datasets = []
    total = min(max_files, len(data_json))  # take only up to max_files samples
    kept = 0
